...
namespace Ikine{
	...
	bool solve(Vector3d position, Quaterniond orientation, double phi, double theta[])
	{
		...
		Vector3d TRz = orientation_mx.col(2); // 期望工具轴姿态
		Vector3d W_dir = TRz * 0.06;
		Vector3d W = position - W_dir; // 期望的wrist position
		// ----- 是否有解 ------
		// 计算腕部距离底座距离
		double dsq = W.dot(W); // 点积 此处等效于sum(W.*W)//求长度平方
		double d = sqrt(dsq);
		...
		if (d > L1 + L2) // 连杆限制腕部距离
		{
			return false; // 无解 d最大L1+L2 最小L1-L2
		}
		// E(C_rot)
		// 求单位Z坐标与单位W/d坐标之间的旋转矩阵
		...
		Vector3d E = Rnorm * C; // 根据腕部姿态旋转冗余圆

		// L
		// Vector3d WE = E - W;
		// Vector3d WE_n = WE.normalized();
		Vector3d E_n = E.normalized();
		Vector3d E_c_W = E.cross(W); // 冗余圆与腕部的法向量 叉积
		...
		Vector3d p_tool = R_04.transpose() * W_dir; // transpose 矩阵转置
